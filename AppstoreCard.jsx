  ...
  return (
    <Card expandable className="appstore-card">
      <CardContent padding={false}>
        <div
          className={`appstore-card-header appstore-card-header-text-${titlePosition}`}
        >
          <img src={image} alt={imageAlt} />
          <div className="appstore-card-header-text">
            <div
              className="appstore-card-subtitle"
              style={{ color: subtitleColor }}
            >
              {subtitle}
            </div>
            <div className="appstore-card-title" style={titleStyle}>
              {title}
            </div>
          </div>
          {app && <AppsTableList style={{ color: appColor }} apps={[app]} />}
        </div>
        <div className="appstore-card-close-button card-opened-fade-in">
          <a href="#" className="link card-close">
            <i style={{ color: closeButtonColor }} className="f7-icons">
